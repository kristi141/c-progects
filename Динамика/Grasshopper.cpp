...
grasshopper can move along the water lilies only one way. In addition, he can 
jump over either one water lily, or two. Every time a grasshopper finds itself 
on water lily, he takes all the grass from it. The grasshopper's goal is to 
...
that, based on these data will display the maximum amount of magic grass that 
can be collected grasshopper reaching the last water lily.

Input data:
The first line of the input file contains a single number N –
number of water lilies (0 ≤ N ≤ 105).
The second line contains N numbers, where each number Ti is equal to the number
grass on the i-th water lily (-109 ≤ Ti ≤ 109).

Output:
The output file must display the total amount of grass that
a grasshopper can collect by jumping over one or two water lilies
just to the right.
